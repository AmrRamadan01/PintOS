...
timer_ticks (void) 
{
  ...
  int64_t t = ticks; //A: inc with timer intrupte
...
int64_t
timer_elapsed (int64_t then) //A: الوقت المنقضي
{
  return timer_ticks () - then; //A: equvilant to millis() in arduino
  //A: millis - previous_reading
  //A: this check if a specific time is passed or not
}

/* Sleeps for approximately TICKS timer ticks.  
  Interrupts must be turned on. (to tick) */

struct semaphore sleeping;
...
{ //this is a wrong implementation should be corrected
  ...
  // and check on it with polling 
  // or you can use semaphores directly
  // it sleeps when you push it to the queue of the blocked threads using semadown()
  // there will be the problem of waking threads up again 
  // so we will use the timer intrrupt
  // this is the place where we will wake up the threads 
